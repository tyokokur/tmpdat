...
//void sovDif_CR(double *in,double **g,double *w,int ns,int sign);
...
int main(int argc, char **argv)
{
	...
	if (vopt == -1) v0 = vcust;
	if (vopt == 0) v0 = 4.0/3.0*M_PI*pow(b0, 3); // approx 4.2 b^3
	...
	if (strstr(seq, "L") != NULL){
		...
		for (j=0; j<K_i[k]; j++){
			...
			orig_Nm = Nm_i[k][j];
			Nm_i[k][j] = round(Nm_i[k][j]*0.360/b0);
		}
...
double getConc(double *phA, double **PHA, double **PHA_T, double *phB, double **WA, double *wB)
{
	...
	//if (iter==100) {write_ph(phA, PHA_T, PHA, phB); exit(0);}
  	///////////////////////////////////////////////////////////
