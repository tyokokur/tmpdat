...
** DONE NFH fit from NFL Candidates
- All thresh 1e-06
...
- [-] Get stable W_ in for 30 and 80 to init for L{1..4}
  - [-] Init v13 from v01 L1_ 30Init from 
    - Init from 8/W_ a* from yesterday (Ns0 1500 1000 sequence)
    - Init from 7/W_ a* recent run, with Ns0 1500 for all
    - Init from finished 8/W_ a*
    - L1_ 30 reinit from self
  - [-] L2_ 30 init from 7/W_ a* from L1_ 30 with Ns0 1500
    - ./8 working best. Reinit all from ./8/W_ a*
    - ./7 working best. Reinit all from ./7/W_ a*
  - [-] L2_ 80 init from 7/W_ a* from L1_ 30 with Ns0 1500
    - ./5 worked. Reinit {1..4} using ./5/W_ a*
    - ./{5,6,8} worked, reinit {1..4, 7} from ./5
...
- L2 (L_ 175)
  - L: b1.75 v2.70 c3.20
  - H: b0.50 v0.13 c3.20
    - L2_ 30/{3..7} little change to NFL H prof
- L3 (L_ 252)
  - L: b1.75 v2.50 c3.20
  - H: b0.50 v0.13 c3.20
- L4 (L_ 272)
  - L: b2.00 v2.70 c3.20
  - H: b0.50 v0.13 c3.20
    - L4_ 30 ./{3..7} seems to be working
    - L4_ 80 see if ./7 converges; redo with wparams 1e-03, 40
